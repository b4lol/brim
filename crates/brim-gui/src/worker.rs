...
pub enum CoreRequest {
    ...
    ImportSync(Vec<brim_core::SyncEntry>),
...
pub enum CoreEvent {
    ...
    SyncParsed(Vec<brim_core::SyncEntry>),
    ...
    rt.block_on(async move {
        ...
        local
            .run_until(async move {
                ...
                let http_client = brim_core::http::client();
                ...
                while let Ok(request) = rx.recv().await {
                    if let CoreRequest::ReloadConfig = request {
                        reload_manager(&manager, &brim_core::Config::load());
                    ...
                    if let CoreRequest::FetchIcon(app_id) = request {
                        ...
                        tokio::task::spawn_local(async move {
                            ...
                            let path = crate::icons::fetch_flathub_icon(&client, &app_id).await;
                            icon_inflight.borrow_mut().remove(&app_id);
...
fn reload_manager(manager: &RefCell<Rc<PackageManager>>, config: &brim_core::Config) {
...
mod tests {
    ...
    fn reload_manager_replaces_the_manager() {
        ...
        reload_manager(&manager, &brim_core::Config::default());
