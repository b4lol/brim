...
use crate::core::backends::{cache, probe, validate_arg, QUERY_TIMEOUT};
...
/// How long a cached flatpak search result stays fresh. `flatpak search`
/// spends ~4-6 s in the system helper; results only change when the
/// remotes' appstream data is refreshed, so a 10-minute TTL is safe.
const SEARCH_CACHE_TTL: Duration = Duration::from_secs(10 * 60);

/// How long the cached pending-updates list stays fresh. `flatpak
/// remote-ls --updates` spends ~5 s in the system helper; the list only
/// changes when remotes refresh their refs or a transaction runs (every
/// flatpak transaction below invalidates this cache).
const UPDATES_CACHE_TTL: Duration = Duration::from_secs(10 * 60);

/// Drop the cached pending-updates list after a transaction changed the
/// installed set (best effort).
async fn invalidate_updates_cache() {
    cache::remove("flatpak-updates", "all").await;
}
...
impl Backend for FlatpakBackend {
    ...
    async fn search(&self, query: &str) -> Result<Vec<Package>> {
        validate_arg(query)?;
        // `flatpak search` spends ~4-6 s in the system helper, so repeat
        // searches are served from a short-lived disk cache first.
        if let Some(cached) = cache::read("flatpak-search", query, SEARCH_CACHE_TTL, |text| {
            !parse_search(text).is_empty()
        })
        .await
        {
            return Ok(parse_search(&cached));
        }
        let out = self
            .run(
                &[
            ...
            .await?;
        cache::write("flatpak-search", query, &out).await;
        Ok(parse_search(&out))
    }
    ...
    async fn install(&self, pkg: &Package) -> Result<TransactionResult> {
        ...
        invalidate_updates_cache().await;
    ...
    async fn updates(&self) -> Result<Vec<Package>> {
        // `remote-ls --updates` spends ~5 s in the system helper; serve
        // repeat calls from a short-lived disk cache. An empty cached
        // list is a valid "no updates" result, so no validity check.
        if let Some(cached) =
            cache::read("flatpak-updates", "all", UPDATES_CACHE_TTL, |_| true).await
        {
            return Ok(parse_updates(&cached));
        }
        let out = self
            .run(
            ...
            .await?;
        cache::write("flatpak-updates", "all", &out).await;
...
mod tests {
    use super::*;
    use crate::core::models::{PackageStatus, RepoKind, SourceType};
