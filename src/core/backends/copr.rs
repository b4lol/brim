...
/// Bound for a single COPR search request. The `project/search` endpoint
/// is slow (~9 s typical) but must never stall a whole search for the
/// client-wide 30 s default.
const SEARCH_TIMEOUT: Duration = Duration::from_secs(15);

/// How long a cached COPR search response stays fresh. COPR projects
/// change slowly; a 10-minute TTL makes repeat searches instant without
/// meaningfully stale results.
const SEARCH_CACHE_TTL: Duration = Duration::from_secs(10 * 60);

/// How long the API reachability probe result is disk-cached. Short
/// enough to notice a network coming back, long enough to keep slow
/// networks from stalling every invocation.
...
impl Backend for CoprBackend {
    ...
    async fn is_available(&self) -> bool {
        ...
        //
        // The probe result is disk-cached for a few minutes: every fan-out
        // waits on this probe before starting real work, so on a slow
        // network it would otherwise stall even sub-second queries on
        // every single invocation.
        if let Some(text) = cache::read("probe", "copr", PROBE_CACHE_TTL, |_| true).await {
            let available = text.trim() == "1";
            let _ = self.available.set(available);
            return available;
        }
        let probe = crate::core::http::get_text(&self.http, COPR_API_ROOT);
        let available = tokio::time::timeout(std::time::Duration::from_secs(5), probe)
            .await
            .map(|r| r.is_ok())
            .unwrap_or(false);
        cache::write("probe", "copr", if available { "1" } else { "0" }).await;
        let _ = self.available.set(available);
        available
    }
    ...
    async fn search(&self, query: &str) -> Result<Vec<Package>> {
        validate_arg(query)?;
        // The endpoint takes ~9 s server-side, so repeat searches are
        // served from a short-lived disk cache first.
        if let Some(cached) = cache::read("copr-search", query, SEARCH_CACHE_TTL, |text| {
        ...
        let out = crate::core::http::get_text_query_timeout(
            &self.http,
            COPR_API_SEARCH,
            &[("query", query)],
            SEARCH_TIMEOUT,
        ...
        .await?;
        cache::write("copr-search", query, &out).await;
        Ok(parse_search(&out))
    }
