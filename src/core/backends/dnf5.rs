...
impl Dnf5Backend {
    ...
    /// Run a read-only dnf5 query in cache-only mode (`-C`), falling back
    /// to a normal run when the cache is missing.
    ///
    /// `-C` skips the metadata freshness check over the network — the
    /// difference between a ~0.5 s and a ~8 s search on a warm cache. The
    /// fallback fires only on cache-miss errors (dnf5's own messages are
    /// English because `run` forces `LC_ALL=C`), so genuinely failing
    /// queries (e.g. `info` on an unknown package) are not needlessly
    /// retried. Transactions never use this path: they refresh metadata
    /// as part of the operation.
    async fn run_query(&self, args: &[&str], allow_update_exit_code: bool) -> Result<String> {
        let cached_args: Vec<&str> = std::iter::once("-C").chain(args.iter().copied()).collect();
        match self
            .run(&cached_args, allow_update_exit_code, Some(QUERY_TIMEOUT))
            .await
        {
            Ok(out) => Ok(out),
            Err(e) if e.to_string().to_lowercase().contains("cache") => {
                self.run(args, allow_update_exit_code, Some(QUERY_TIMEOUT))
                    .await
            }
            Err(e) => Err(e),
        }
...
impl Backend for Dnf5Backend {
    ...
    async fn search(&self, query: &str) -> Result<Vec<Package>> {
        validate_arg(query)?;
        let out = self.run_query(&["search", "-q", query], false).await?;
        Ok(parse_search(&out))
    }

    async fn list_installed(&self) -> Result<Vec<Package>> {
        let out = self
            .run_query(&["list", "--installed", "-q"], false)
            .await?;
    ...
    async fn info(&self, id: &str) -> Result<Package> {
        validate_arg(id)?;
        let out = self.run_query(&["info", id], false).await?;
    ...
    async fn updates(&self) -> Result<Vec<Package>> {
        let out = self.run_query(&["check-update", "-q"], true).await?;
...
mod tests {
    use super::*;
    use crate::core::models::{Category, PackageStatus, SourceType};
