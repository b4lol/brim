...
/// Run the web UI and REST API on 127.0.0.1:`port`. Serves until the
/// process is killed; startup failures are reported and exit nonzero.
...
fn token_file_path() -> PathBuf {
    let dir = match std::env::var_os("XDG_RUNTIME_DIR") {
        Some(runtime_dir) if !runtime_dir.is_empty() => PathBuf::from(runtime_dir).join("brim-web"),
