...
use crate::gui::rows;
use crate::gui::worker::{self, CoreEvent, CoreRequest};
...
pub fn build(app: &adw::Application) {
    ...
    let shared_config = Rc::new(RefCell::new(crate::core::Config::load()));
...
fn add_repo_groups(page: &adw::PreferencesPage, request_tx: &Sender<CoreRequest>) -> RepoGroups {
    ...
    add_button.set_valign(Align::Center);
    {
        ...
        add_button.connect_clicked(move |button| {
            ...
            if tx
                .try_send(CoreRequest::AddFlatpakRemote(name, url))
                .is_err()
            {
...
fn fill_repo_groups(
    ...
    for repo in repos {
        let row = adw::ActionRow::builder()
            ...
            .build();
        match repo.kind {
            crate::core::RepoKind::FlatpakRemote => {
